...
import world.player.skill.smithing.smithBars.ANVIL
import world.player.skill.smithing.smithBars.smithTable

enum class BronzeTable(item: Item, level: Int, bars: Int, lines: Pair<Int, Int>) {


    companion object {
        val BAR = Bar.BRONZE.id
        val BASE_EXP = 12.5
    }
}

val bronzeTable =
    smithTable(bar = Bar.BRONZE,
               exp = 12.5,
               ) {
        item("Bronze dagger")
    }
